...
test_that("build_request creates valid httr2 request", {
  req <- CDCdata:::build_request("25m4-6qqq")
...
test_that("build_request sets JSON accept header by default", {
  req <- CDCdata:::build_request("25m4-6qqq", format = "json")
...
test_that("build_request2 creates valid httr2 request", {
  req <- CDCdata:::build_request2("/resource/test-1234.json")

  expect_s3_class(req, "httr2_request")
})
test_that("build_request2 sets correct base URL", {
  req <- CDCdata:::build_request2("/resource/test-1234.json")

  expect_match(req$url, "https://data.cdc.gov")
})

test_that("build_request2 sets JSON accept header by default", {
  req <- CDCdata:::build_request2("/resource/test-1234.json", format = "json")

  # Headers are stored in the request
  expect_true(!is.null(req$headers))
})

test_that("build_request2 sets CSV accept header when format='csv'", {
  req <- CDCdata:::build_request2("/resource/test-1234.csv", format = "csv")
...
test_that("cdc_get_token returns NULL when not set", {
  old_token <- Sys.getenv("CDC_APP_TOKEN", unset = NA)
  on.exit({
    if (is.na(old_token)) {
      Sys.unsetenv("CDC_APP_TOKEN")
  ...
  expect_null(CDCdata:::cdc_get_token())
})

test_that("cdc_get_token returns token when set", {
  old_token <- Sys.getenv("CDC_APP_TOKEN", unset = NA)
  on.exit({
    if (is.na(old_token)) {
      Sys.unsetenv("CDC_APP_TOKEN")
  ...
  expect_equal(CDCdata:::cdc_get_token(), "my-test-token")
})
